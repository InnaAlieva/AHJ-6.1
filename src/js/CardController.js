...
`;
	...
	onClickAddCard() {
		...
		this.addCardBtn.forEach((item) => {
			item.addEventListener('click', () => {
				...
				item.parentElement.insertAdjacentHTML('beforeend', this.constructor.creatingCard);
	...
	onClickPinCard(e) { // закрепить карточку на доске
		e.preventDefault();
		this.card = new Card();
	...
	dragUp(e) {
		...
		const closest = document.elementFromPoint(e.clientX, e.clientY).closest('.pinned__card');
